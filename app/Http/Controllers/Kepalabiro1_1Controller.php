...
class Kepalabiro1_1Controller extends Controller
{
    // Menampilkan daftar dokumen yang menunggu Kepala Biro 1
    public function index()
    {
        $dokumen = Dokumen::where('status_verifikasi', 'Menunggu Kepala Biro 1')->get();
        return view('Kepalabiro1_1.index', compact('dokumen'));
    }
    ...
    public function setujui(Request $request, $id)
    {
        ...
        // Ambil nama Kepala Biro 1 aktif
        $namaKepalaBiro = DB::table('users')
            ->where('role', 'Kepala_Biro_1')
            ->value('name');

        // Simpan ke histori verifikasi
        DB::table('histori_verifikasi')->insert([
            'dokumen_id' => $id,
            'posisi' => 'Kepala Biro 1',
            'jurubeli' => $namaKepalaBiro,
            'tanggal_masuk' => $tanggalMasuk,
            'tanggal_keluar' => now(),
            'catatan' => "Disetujui oleh Kepala Biro 1 dan diteruskan ke Kepala Departemen.",
        ]);
    ...
    public function tolak(Request $request, $id)
    {
        ...
        $dokumen->status_verifikasi = 'Ditolak Kepala Biro 1';
        ...
        return redirect()->route('Kepalabiro1_1.index')
            ->with('error', 'Dokumen ditolak dan dikembalikan ke Juru Beli 1.');
    }
}