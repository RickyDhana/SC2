...
<head>
    ...
    <title>Verifikasi Dokumen - Verifikator 2</title>
...
<body class="bg-[#0a1730] text-white min-h-screen">
    ...
    <header class="bg-gradient-to-r from-[#1c2e55] to-[#0a1730] shadow-md p-4 flex justify-between items-center">
        ...
        <a href="{{ route('v2.index') }}" class="text-sm text-white hover:underline">← Kembali</a>
    </header>
    ...
    <main class="p-6">
        <div class="bg-white text-gray-900 p-6 rounded-lg shadow-lg max-w-2xl mx-auto">
            ...
            <div class="mt-6 flex gap-4">
                {{-- Tombol Setujui --}}
                <form action="{{ route('v2.setujui', $dokumen->id) }}" method="POST">
                    @csrf
                    <button type="submit"
                ...
                {{-- Tombol Tolak --}}
                <form action="{{ route('v2.tolak', $dokumen->id) }}" method="POST" class="flex-1">
                    @csrf
